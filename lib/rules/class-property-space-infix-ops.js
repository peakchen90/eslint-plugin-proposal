module.exports = {
  meta: {
    docs: {
      ...
      recommended: false,
      url: 'https://github.com/peakchen90/eslint-plugin-proposal/blob/master/docs/rules/class-property-space-infix-ops.md'
    },
    fixable: 'whitespace',
    schema: [{
      type: 'string',
    ...
    messages: {
      spacing: 'Operator \'=\' must be spaced.',
      nonSpacing: 'Operator \'=\' should not be spaced.'
    }
  },
  ...
  create(context) {
    ...
    function report(node, operator) {
      let messageId = 'spacing';
      let loc = operator.loc;
      if (spaceSize === 0) {
        messageId = 'nonSpacing';
    ...
    return {
      ClassProperty(node) {
        // do not handle uninitialized class property
        if (node.value !== null) {
